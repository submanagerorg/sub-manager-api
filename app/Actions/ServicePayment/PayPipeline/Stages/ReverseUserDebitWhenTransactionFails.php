    ...
    public function handle(HandlePaymentState $state, Closure $next)
    {
        if ($state->transactionFailed()) {
            ...
            if ($state->transactionFailed() && $user) {
                $user->wallet->credit($this->generateReference(Wallet::LABEL), $requestData['full_amount'], 0, WalletTransaction::TYPE['REVERSAL'], ucwords($requestData['service_name']) . ' Payment Reversal');
